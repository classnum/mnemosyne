...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*:l[not(@n) or (@n = 'NaN') or (@n = '')]">
        <xsl:variable name="container" select="./ancestor::*[@type='edition']" />
        <xsl:variable name="seq" select="$container//*:l" />
        <xsl:variable name="current" select="." />
        <xsl:variable name="prev" select="$seq[my:index-of($seq, $current)[1] -1]"/>
    ...
    <!--<xsl:template name="poemloop" match="*:div[@type='edition' and count(.//*:l[not(@n) or (@n = 'NaN') or (@n = '')])]" saxon:threads="8">
