...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--
        This XSLT has been developed to complete the line numbers in existing and partially numbered texts.

        # How to use
        - Before running this XSLT, I advise to :
            - look at the text
            - number manually parts of the texts that have interchanged number (i.e. if line ares numbered [5 - 8 - - 10], you should number them [5 6 8 9 10])
    ...
    <!-- 2018-05-14 Aurélien Berra: adapt to dramatic corpora (Aristophanes) encoded with sp and speaker tags
        Fatal error: Saxon-HE 9.7.0.19 "Too many nested apply-templates calls. The stylesheet may be looping." Start location: 69:43
        Decreasing or removing saxon:threads doesn't help.
    -->

    <xsl:template match="*:l[not(@n) or (@n = 'NaN') or (@n = '')]">
        <xsl:variable name="container" select="./ancestor::*:div[@type = 'edition']"/>
        <xsl:variable name="seq" select="$container//*:l"/>
        <xsl:variable name="prev" select="preceding-sibling::*:l[position() = 1]"/>

        <xsl:variable name="prev-sp-last-l"
            select="../preceding-sibling::*:sp[position() = 1]/*:l[last()]"/>

        <xsl:message>
            <xsl:value-of select="not(./@n = 'NaN') or not(./@n = '')"/>
        </xsl:message>
        <xsl:message>
        ...
        <xsl:copy>
            ...
            <xsl:choose>
                ...
                <!-- adding step: when no previous line, capture @n value of last line in preceding <sp> -->
                <xsl:when test="count($prev) = 0 and count($prev-sp-last-l) = 1">
                    <xsl:message>
                        <xsl:copy-of select="$prev-sp-last-l"/>
                    </xsl:message>
                    <xsl:attribute name="n" select="number($prev-sp-last-l/@n) + 1"/>
                </xsl:when>
                <xsl:when test="count($prev) = 0 and count($prev-sp-last-l) = 0">
    ...
    <xsl:template name="poemloop"
        match="*:div[@type = 'edition' and count(.//*:l[not(@n) or (@n = 'NaN') or (@n = '')])]"
        saxon:threads="8">
        <!-- ORIGINAL TEMPLATE
